...
fmax = 10
# VTF filename 
vtffilename = "data/03/ljsim.vtf"
# DATA filename 
datafilename = "data/03/ljsim.dat"
# STATE filename 
statefilename = "data/03/ljsim.state"
# COMPUTED CONSTANTS
...
L = volume**(1./3.)
# damping constant for Langevin dynamics
gamma = 0.5
...
def force_ld(v):
    global gamma, Tdes
    if FORCECAPPING: return 0
    elif LD: return -gamma*v+np.sqrt(2*gamma*Tdes)*np.random.normal(0,1,v.shape[1])*10 
    # ToDo - Probleme Lösen:
    # Bei der 10 handelt es sich um einen Hack. Alternative ließe sich auch die Energie mit 100 multiplizieren. Eventuell wurde die Energie irgendwie falsch berechnet.
    # LD verträgt sich nicht mit RANDOMPOSITION = True
